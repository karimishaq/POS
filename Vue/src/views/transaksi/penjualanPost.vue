      ...
      <v-card-text>
        ...
        <form @submit.prevent="store">
          <div class="form-group">
            ...
            <input type="text" v-model="post.Pelanggan" placeholder="Masukkan Supplier">
          </div>
...
<script>
  ...
  export default{
    setup(){
      ...
      const kolomku=["Nama", "Kuantitas", "Harga", "Diskon", ""]
      const post=reactive({
        Id_Penjualan:'',
        Pelanggan:'',
        Operator:'',
      ...
      function tambah_item(data){
        let o=data_transaksi.value.find((o)=>o.id==data.id);
        if(!o)
      ...
      function store(){
        ...
        api.post('api/penjualan', {id:id, Pelanggan:post.Pelanggan, Operator:'Anonim', Harga:post.Harga, Keterangan:post.Keterangan}).then(() => {        
          api.post('api/item_penjualan', {id:id, barang:data_transaksi.value}).then(()=>{dialog.value=false;});
        })
      }
