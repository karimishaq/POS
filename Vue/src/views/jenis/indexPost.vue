<template>
  <main>
    <v-row>
      <v-col>
        <v-card>
          <v-card-title>Data Jenis</v-card-title>
          <HR />
          <v-card-text>
                ...
                <tr v-for="(data, index) in dataku" :key="index">
                  ...
                  <td>
                      <div class="kolom">
                          ...
                          <router-link :to="{name: 'jenis.edit', params: { id: data.id }}"><v-btn size="small" color="warning"><v-icon>mdi-pencil</v-icon></v-btn></router-link>
                      </div>
                  </td>
                </tr>
              </tbody>
      ...
      <v-card-text>
        ...
        <form @submit.prevent="store">
            <div class="form-group">
              ...
              <input type="text" v-model="post.Nama" placeholder="Masukkan Nama jenis">
            </div>
...
<script>
    ...
    export default{
      ...
      setup(){
        ...
        function getDataku(){
            api.get('api/jenis', {params:{awal:(jtampil.value*(paginasi.value-1)), total:jtampil.value, pencarian:pencarian.value}}).then(response => {
